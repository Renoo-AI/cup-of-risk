    ...
    <div className={`w-full h-full flex flex-col items-center justify-between py-2 px-2 sm:p-8 bg-zinc-950 animate-in fade-in duration-300 ${isResolving ? 'pointer-events-none' : ''}`}>
      ...
      <div className="w-full flex justify-between items-center max-w-5xl gap-1 sm:gap-2 mt-1 sm:mt-2">
        <div className={`flex flex-col items-center p-1.5 sm:p-6 rounded-xl sm:rounded-[2rem] transition-all border-2 sm:border-4 ${
          currentPlayerIdx === 0 
          ? 'bg-blue-600/20 border-blue-500 scale-105 shadow-[0_0_30px_rgba(59,130,246,0.5)] animate-[pulse-glow-blue_2s_infinite]'
          : 'bg-zinc-900/50 border-zinc-800 opacity-50'
        }`}>
          <span className="font-game text-[9px] sm:text-3xl text-blue-500 mb-0.5">P1</span>
          <div className="flex gap-0.5 bg-gradient-to-r from-blue-500/10 to-transparent rounded-full px-2">
            {[...Array(players[0].lives)].map((_, i) => (
              <span key={i} className="text-xs sm:text-3xl drop-shadow-[0_0_10px_rgba(239,68,68,0.5)]">❤️</span>
            ))}
        ...
        <div className={`flex flex-col items-center p-1.5 sm:p-6 rounded-xl sm:rounded-[2rem] transition-all border-2 sm:border-4 ${
          currentPlayerIdx === 1 
          ? 'bg-red-600/20 border-red-500 scale-105 shadow-[0_0_30px_rgba(239,68,68,0.5)] animate-[pulse-glow-red_2s_infinite]'
          : 'bg-zinc-900/50 border-zinc-800 opacity-50'
        }`}>
          <span className="font-game text-[9px] sm:text-3xl text-red-500 mb-0.5">P2</span>
          <div className="flex gap-0.5 bg-gradient-to-r from-red-500/10 to-transparent rounded-full px-2">
      ...
      <div className="relative mt-1 sm:mt-2">
        {/* Stage Light Effect */}
        <div className="absolute -top-10 inset-x-0 h-32 bg-gradient-to-b from-yellow-400/10 to-transparent blur-3xl pointer-events-none z-0"></div>

        <div className="relative z-10 grid grid-cols-4 gap-1 sm:gap-6 p-2 sm:p-12 bg-zinc-900/40 rounded-[1.2rem] sm:rounded-[3rem] border-b-4 sm:border-b-[12px] border-zinc-800/50">
          {cups.map((cup) => (
            <div 
              key={cup.id} 
